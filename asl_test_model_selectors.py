...
class TestSelectors(TestCase):
    ...
    def test_select_bic_interface(self):
        model = SelectorBIC(self.sequences, self.xlengths, 'FRANK',verbose=True).select()
        self.assertGreaterEqual(model.n_components, 2)
        model = SelectorBIC(self.sequences, self.xlengths, 'VEGETABLE',verbose=True).select()
        self.assertGreaterEqual(model.n_components, 2)
        #model = SelectorBIC(self.sequences, self.xlengths, 'BOOK').select()
        #self.assertGreaterEqual(model.n_components, 2)
